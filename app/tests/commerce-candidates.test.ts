...
describe('buildShopCandidates (Canto Alto real)', () => {
  const built = buildShopCandidates({
    ...
    imbuicoes: [byName('Imbuição Relampejante'), byName('Imbuição Flamejante')],
  ...
  it('combo com imbuição INCOMUM (Flamejante ∉ Recursos) → ×¼', () => {
    expect(find('Adaga Flamejante')!.mult).toBe(0.25)
  })
