...
function FolderNote({
  ...
  if (!doc || !dedicated) {
    ...
    // Report 2026-08-29: índice PURO (sem corpo útil) → título é TEXTO, não
    // link — a página da nota só re-renderizava as transclusões em parede de
    // texto. Com corpo útil o título segue linkável (abre a nota completa).
    return (
      <>
        <h1>{showBody ? <Link to={docPath(id)}>{fallbackTitle}</Link> : fallbackTitle}</h1>
