// PAINEL DA MALHA (2026-09-09) — tudo que a malha de transportes oferece SEM
// depender de qual mapa a desenha: o filtro (cartão TRI, modo, "só este
// cartão", "mostrar o que não abre"), o planejador de trajeto com itinerário
// passo a passo, a legenda das linhas e o parada a parada.
//
// Nasceu como a aba TRANSPORTE da ficha (2026-09-08) e virou painel quando o
// Atlas ganhou a MESMA malha desenhada sobre o mapa real de Porto Alegre: o
// mapa entra por render prop (`mapa`), o resto é um só — não existem duas
// cópias das capacidades. A ficha passa o `heroi` (plano do FM) e desenha o
// mapa esquemático; o Atlas não passa herói e desenha as linhas no mapa real.
//
// Tudo vem da vault: notas de Linha, bloco ```malha``` da nota do mapa, planos
...
import { abaDoPapel } from '../recursos/hero-recursos'
...
export interface ContextoMapaMalha {
  ...
  visiveis: LinhaMalha[]
  /** Linhas de fora do filtro, quando o jogador pediu pra ver o que não abre. */
  bloqueadas: Set<string> | undefined
  /** Projeção esquemática (grade do bloco ```malha```) das linhas visíveis. */
  desenho: Desenho
  /** Territórios de bairro na projeção esquemática. */
  bairros: ZonaBairro[]
...
export interface HeroiDaMalha {
  /** Nível do estilo de vida do eixo transporte (1 = sem plano). */
  nivel: number
  /** Nome do plano no FM (pode não ser cartão: "A Pé", carro com motorista). */
  plano: string | null
}

export function MalhaPainel({ heroi, mapa }: { heroi?: HeroiDaMalha; mapa: (ctx: ContextoMapaMalha) => ReactNode }) {
  ...
  const planoAtual = heroi?.plano ?? null
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {/* NA FICHA: o cartão TRI (transporte coletivo — carro e táxi ficam na
          aba Recursos). No Atlas não há herói, então não há cartão a mostrar. */}
      {heroi ? (
      ...
      {mapa({
        malha,
        visiveis,
        bloqueadas: verBloqueadas ? new Set(bloqueadas.map((l) => l.id)) : undefined,
        desenho,
        bairros,
        selecionada,
        destaque,
